...
* @author  KIM SUNWOO
* @Mod		 2023-10-27
* @brief   Embedded Controller:  EC_HAL_for_ecPWM
*
******************************************************************************
*/
...
void PWM_init(PinName_t pinName) {
  ...
  GPIO_otype(port, pin, EC_OUT_PU); 	//if necessary
	GPIO_pupd(port, pin, EC_NO); 					//if necessary
...
// 2. Configure GPIO AFR by Pin num.	
	...
	if	(pin<8) {
		if 			(TIMx == TIM1 || TIMx == TIM2) 									 port->AFR[0] |= 0x01 << (4*pin);
		else if (TIMx == TIM3 || TIMx == TIM4 || TIMx == TIM5)   port->AFR[0] |= 0x02 << (4*pin);
		else if (TIMx == TIM9 || TIMx == TIM10 || TIMx == TIM11) port->AFR[0] |= 0x03 << (4*pin);
	}
	else {
		if 			(TIMx == TIM1 || TIMx == TIM2) 									 port->AFR[1] |= 0x01 << (4*(pin%8));
		else if (TIMx == TIM3 || TIMx == TIM4 || TIMx == TIM5)   port->AFR[1] |= 0x02 << (4*(pin%8));
		else if (TIMx == TIM9 || TIMx == TIM10 || TIMx == TIM11) port->AFR[1] |= 0x03 << (4*(pin%8));
	}
