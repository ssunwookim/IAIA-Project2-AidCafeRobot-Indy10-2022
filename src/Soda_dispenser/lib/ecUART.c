...
//1. GPIO Pin for TX and RX	
	...
	GPIO_otype(GPIO_TX, pinTX, EC_OUT_PU);
	GPIO_pupd(GPIO_TX, pinTX, EC_NO);		
	GPIO_ospeed(GPIO_TX, pinTX, EC_HIGH);						
	
	GPIO_init(GPIO_RX, pinRX, AF);
	GPIO_otype(GPIO_RX, pinRX, EC_OUT_PU);
	GPIO_pupd(GPIO_RX, pinRX, EC_NO);
...
// 3. Read USARTx Data (Interrupt)	
	...
	if (USARTx == USART1){
		NVIC_SetPriority(USART1_IRQn, 10);      		// Set Priority to 1
		NVIC_EnableIRQ(USART1_IRQn);             	// Enable interrupt of USART2 peripheral
	}
	else if (USARTx == USART2){
		NVIC_SetPriority(USART2_IRQn, 10);      		// Set Priority to 1
		NVIC_EnableIRQ(USART2_IRQn);             	// Enable interrupt of USART2 peripheral
	}
	else {																			// if(USARTx==USART6)
		NVIC_SetPriority(USART6_IRQn, 10);      		// Set Priority to 1
