...
* @author  KIM SUNWOO
* @Mod		 2023-10-27
* @brief   Embedded Controller:  EC_HAL_for_ecICPA
*
******************************************************************************
*/
...
// 1. Initialize Timer Interrpt 
	TIM_UI_init(TIMx, 50);        					// TIMx Interrupt initialize 
...
// 1. Select Timer channel(TIx) for Input Capture channel(ICx)
	...
	TIMx->CCMR1 |= 	TIM_CCMR2_CC3S_0;       				//01<<0   CC3s    TI3=IC3
	TIMx->CCMR1 |= 	TIM_CCMR2_CC4S_0;							//01<<8   CC4s    TI4=IC4
...
// 4. Activation Edge: CCyNP/CCyP	
	TIMx->CCER &= ~(0b1010 << 4*(ICn - 1));					// CCy(Rising) for ICn , CCn(ICn) Capture Enable	
