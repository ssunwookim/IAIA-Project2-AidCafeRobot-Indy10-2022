...
class CameraNode1: # Change Node Name (Important)

    def __init__(self):
        rospy.init_node('camera_node1', anonymous=True)  # 노드 이름 "camera_node"로 초기화, Change Node Name (Important)
        self.bridge = CvBridge()                # cv_bridge 객체 생성

        # "camera/image_raw"라는 토픽으로 메시지를 publish할 publisher 객체 생성
        self.image_pub = rospy.Publisher("camera/image_raw1",Image,queue_size=1) # Change Node Name (Important)
...
if __name__ == '__main__':
    try:
        camera = CameraNode1()       # CameraNode 객체 생성, Change Node Name (Important)
